...
val pathfinderCreator: PathfinderCreator =
    nmsProxy(NeonLibsLoader.getInstance(),"{name}Impl${if (MinecraftVersion.isUniversal) "17" else ""}")




val pathfinderExecutor: PathfinderExecutor =
    nmsProxy(NeonLibsLoader.getInstance(),"{name}Impl" + if (MinecraftVersion.isUniversal) "17" else "")
