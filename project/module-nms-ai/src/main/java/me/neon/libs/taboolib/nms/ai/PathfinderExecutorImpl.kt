package me.neon.libs.taboolib.nms.ai

import me.neon.libs.taboolib.nms.MinecraftVersion
import me.neon.libs.taboolib.nms.MinecraftVersion.major
...
class PathfinderExecutorImpl : PathfinderExecutor() {
    ...
    override fun controllerLookAt(entity: LivingEntity, target: Location) {
        val controllerLook = (getControllerLook(entity) as ControllerLook)
        controllerLook.a(target.x, target.y, target.z, 10f, 40f)
        val nms = getEntityInsentient(entity) as EntityInsentient
        if (nms.isNoAI) {
            controllerLook.a()
        }
    }
    ...
    override fun controllerLookAt(entity: LivingEntity, target: Entity) {
        ...
        val controllerLook = (getControllerLook(entity) as ControllerLook)
        controllerLook.a((target as CraftEntity).handle, 10f, 40f)
        val nms = getEntityInsentient(entity) as EntityInsentient
        if (nms.isNoAI) {
            controllerLook.a()
        }
