...
fun HumanEntity.openVirtualInventory(inventory: VirtualInventory, updateId: Boolean = true): RemoteInventory {
    val remoteInventory = InventoryHandlers.instance.openInventory(this as Player, inventory, ItemStack(Material.AIR), updateId)
    inventory.remoteInventory = remoteInventory
    InventoryHandlers.playerRemoteInventoryMap[name] = remoteInventory
    // 唤起事件
    if (Bukkit.isPrimaryThread()) {
        Bukkit.getPluginManager().callEvent(InventoryOpenEvent(remoteInventory.createInventoryView()))
    } else {
        syncRunner { Bukkit.getPluginManager().callEvent(InventoryOpenEvent(remoteInventory.createInventoryView())) }
    }
