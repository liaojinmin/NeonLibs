...
abstract class CommandComponent(val index: Int, var optional: Boolean, val permission: String = "") {
     ...
     * @param function 执行函数
     */
    fun execute(bind: Class<CommandSender>, function: (sender: CommandSender, context: CommandContext, argument: String) -> Unit) {
     ...
     * @param function 执行函数
     */
    fun execute(function: (sender: CommandSender, context: CommandContext, argument: String) -> Unit) {
        execute(CommandSender::class.java, function)
    }
