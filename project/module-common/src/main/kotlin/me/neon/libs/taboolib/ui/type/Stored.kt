    ...
    override fun build(): Inventory {
        ...
        selfClick {
            ...
            // 处理点击事件
                ...
                // 自动装填
                if (it.clickEvent().click.isShiftClick && it.rawSlot >= it.inventory.size && (currentItem != null && currentItem.type != Material.AIR)) {
                    it.isCancelled = true
                    // 获取有效位置
                    val firstSlot = rule.firstSlot(it.inventory, currentItem)
                    // 目标位置不存在任何物品
                    // 防止覆盖物品
                    if (firstSlot >= 0 && rule.readItem(it.inventory, firstSlot)?.type == Material.AIR) {
                        // 设置物品
