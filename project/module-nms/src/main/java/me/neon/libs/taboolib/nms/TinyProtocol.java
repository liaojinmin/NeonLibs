...
public class TinyProtocol {
    ...
    private static final TinyReflection.MethodInvoker GET_PLAYER_HANDLE = TinyReflection.getMethod("{obc}.entity.CraftPlayer", "getHandle");
    private static final TinyReflection.FieldAccessor<?> GET_CONNECTION = TinyReflection.getField(ENTITY_PLAYER_CLASS, null, PLAYER_CONNECTION_CLASS);
    private static final TinyReflection.FieldAccessor<?> GET_MANAGER = TinyReflection.getField(PLAYER_CONNECTION_CLASS, null, NETWORK_MANAGER_CLASS);
    private static final TinyReflection.FieldAccessor<Channel> GET_CHANNEL = TinyReflection.getField(NETWORK_MANAGER_CLASS, Channel.class, 0);
    ...
    private static final TinyReflection.FieldAccessor<Object> GET_MINECRAFT_SERVER = TinyReflection.getField("{obc}.CraftServer", MINECRAFT_SERVER_CLASS, 0);
    private static final TinyReflection.FieldAccessor<Object> GET_SERVER_CONNECTION = TinyReflection.getField(MINECRAFT_SERVER_CLASS, SERVER_CONNECTION_CLASS, 0);
    ...
    private static TinyReflection.FieldAccessor<String> getName;
    private static TinyReflection.FieldAccessor<GameProfile> getGameProfile;

    // Speedup channel lookup
    private final Map<String, Channel> channelLookup = new MapMaker().weakValues().makeMap();
    private Listener listener;

    // Channels that have already been removed 已被移除的频道
    private final Set<Channel> uninjectedChannels = Collections.newSetFromMap(new MapMaker().weakKeys().makeMap());

    // 注入携带程序
    private final Map<Plugin, BiFunction<Player, Channel, Void>> injectFunction = new HashMap<>();

    private final Map<Plugin, BiFunction<Player, Channel, Void>> uninjectFunction = new HashMap<>();
    ...
    public void registerInject(Plugin plugin, BiFunction<Player, Channel, Void> function) {
        injectFunction.put(plugin, function);
    }

    public void registerUninject(Plugin plugin, BiFunction<Player, Channel, Void> function) {
        uninjectFunction.put(plugin, function);
    }
        ...
        endInitProtocol = new ChannelInitializer<Channel>() {
            ...
            protected void initChannel(Channel channel) {
                try {
                    ...
                    NeonLibsLoader.warning("Cannot inject incoming channel " + channel);
                    e.printStackTrace();
                }
            }
        };
    ...
    private void registerBukkitEvents() {
        ...
        Bukkit.getServer().getPluginManager().registerEvents(listener, NeonLibsLoader.getInstance());
    }
    ...
    private void registerChannelHandler() {
        ...
        try {
            ...
            NeonLibsLoader.warning("Encountered an exception checking list fields");
            ex.printStackTrace();
            TinyReflection.MethodInvoker method = TinyReflection.getTypedMethod(SERVER_CONNECTION_CLASS, null, List.class, SERVER_CONNECTION_CLASS);
    ...
    protected String getHandlerName() {
        return "tiny-" + NeonLibsLoader.getInstance().getName() + "-" + ID.incrementAndGet();
    }
    ...
    public void injectPlayer(Player player) {
        ...
        injectFunction.values().forEach(it -> it.apply(player, channel));
    ...
    public void uninjectPlayer(Player player) {
        Channel channel = getChannel(player);
        uninjectFunction.values().forEach(it -> it.apply(player, channel));
    ...
    private final class PacketInterceptor extends ChannelDuplexHandler {
        ...
        public void channelRead(ChannelHandlerContext ctx, Object msg) throws Exception {
            ...
            try {
                ...
                NeonLibsLoader.warning("Error in onPacketInAsync()");
                e.printStackTrace();
            }
            if (msg != null) {
        ...
        public void write(ChannelHandlerContext ctx, Object msg, ChannelPromise promise) throws Exception {
            try {
                ...
                NeonLibsLoader.warning("Error in onPacketOutAsync()");
                e.printStackTrace();
            }
            if (msg != null) {
