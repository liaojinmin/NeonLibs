...
import me.neon.libs.util.item.ItemBuilder
import me.neon.libs.util.item.buildItem
import me.neon.libs.util.item.modifyMeta
...
class NMSMap(val image: BufferedImage, var hand: Hand = Hand.MAIN, val builder: ItemBuilder.() -> Unit = {}) {
    ...
    val mapItem by unsafeLazy {
        val map = if (MinecraftVersion.isHigherOrEqual(MinecraftVersion.V1_13)) {
            buildItem(Material.FILLED_MAP, builder)
        } else {
            buildItem(Material.FILLED_MAP) {
    ...
    fun sendTo(player: Player) {
        syncRunner(3) {
            ...
            player.sendAsyncPacket(classPacketPlayOutSetSlot.unsafeInstance().also {
