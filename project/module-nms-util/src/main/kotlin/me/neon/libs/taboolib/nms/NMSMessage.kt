...
fun BossBar.setRawTitle(title: String) {
    nmsProxy<NMSMessage>(NeonLibsLoader.getInstance()).setRawTitle(this, title)
}
...
fun Player.sendRawTitle(title: String?, subtitle: String?, fadein: Int = 0, stay: Int = 20, fadeout: Int = 0) {
    nmsProxy<NMSMessage>(NeonLibsLoader.getInstance()).sendRawTitle(this, title, subtitle, fadein, stay, fadeout)
}
...
fun Player.sendRawActionBar(message: String) {
    nmsProxy<NMSMessage>(NeonLibsLoader.getInstance()).sendRawActionBar(this, message)
}
...
class NMSMessageImpl : NMSMessage() {
    ...
    override fun sendRawTitle(player: Player, title: String?, subtitle: String?, fadein: Int, stay: Int, fadeout: Int) {
        ...
        if (MinecraftVersion.isUniversal) {
            player.sendAsyncPacket(NMSClientboundSetTitlesAnimationPacket(fadein, stay, fadeout))
            if (title != null) {
                player.sendAsyncPacket(NMSClientboundSetTitleTextPacket(NMSChatSerializer.fromJson(title)))
            }
            if (subtitle != null) {
                player.sendAsyncPacket(NMSClientboundSetSubtitleTextPacket(NMSChatSerializer.fromJson(subtitle)))
            }
        } else {
            player.sendAsyncPacket(NMSPacketPlayOutTitle16(fadein, stay, fadeout))
            if (title != null) {
                // 1.16+
                // ChatSerializer.a 的返回值由 IChatBaseComponent 变为 IChatMutableComponent
                if (MinecraftVersion.isHigherOrEqual(MinecraftVersion.V1_16)) {
                    player.sendAsyncPacket(NMSPacketPlayOutTitle16(NMSEnumTitleAction16.TITLE, NMSChatSerializer16.a(title)))
                } else {
                    player.sendAsyncPacket(NMSPacketPlayOutTitle8(NMSEnumTitleAction8.TITLE, NMSChatSerializer8.a(title)))
                }
            }
            if (subtitle != null) {
                if (MinecraftVersion.isHigherOrEqual(MinecraftVersion.V1_16)) {
                    player.sendAsyncPacket(NMSPacketPlayOutTitle16(NMSEnumTitleAction16.SUBTITLE, NMSChatSerializer16.a(subtitle)))
                } else {
                    player.sendAsyncPacket(NMSPacketPlayOutTitle8(NMSEnumTitleAction8.SUBTITLE, NMSChatSerializer8.a(subtitle)))
                }
            }
        }
    }
    ...
    override fun sendRawActionBar(player: Player, action: String) {
        try {
            ...
            player.sendAsyncPacket(NMSPacketPlayOutChat16().also {
