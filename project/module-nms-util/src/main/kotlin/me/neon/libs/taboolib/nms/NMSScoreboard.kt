...
import me.neon.libs.taboolib.nms.type.ChatColorFormat
import me.neon.libs.taboolib.nms.type.PlayerScoreboard
import me.neon.libs.util.*
...
class NMSScoreboardImpl : NMSScoreboard() {
    ...
    override fun updateTeam(player: Player, prefix: String, suffix: String, color: ChatColorFormat, created: Boolean, target: Player?) {
        ...
        if (MinecraftVersion.isHigherOrEqual(MinecraftVersion.V1_13)) {
            ...
            if (target == null) {
                Bukkit.getOnlinePlayers().forEach { pp -> pp.sendAsyncPacket(packet) }
            } else target.sendAsyncPacket(packet)
            return
        }
        val packet = net.minecraft.server.v1_16_R3.PacketPlayOutScoreboardTeam()
        packet.setProperty("a", player.displayName)
    ...
    private fun createTeam(player: Player) {
        ...
        if (MinecraftVersion.isHigherOrEqual(MinecraftVersion.V1_13)) {
            ...
            packet.setProperty("i", 0)
            packet.setProperty("j", -1)
            Bukkit.getOnlinePlayers().forEach { p -> p.sendAsyncPacket(packet) }
            return
        }
        val packet = net.minecraft.server.v1_16_R3.PacketPlayOutScoreboardTeam()
        packet.setProperty("a", player.displayName)
    ...
    private fun updateLineCount(player: Player, line: Int, lastLineCount: Int) {
        ...
        if (line > lastLineCount) {
            (lastLineCount until line).forEach { i ->
                ...
                if (MinecraftVersion.majorLegacy > 12002) {
                    player.sendAsyncPacket(PacketPlayOutScoreboardScore::class.java.invokeConstructor(uniqueOwner[i], objectiveName, i, null, null))
                    return@forEach
                }
                // 1.13+ 直接实例化
                if (MinecraftVersion.isHigherOrEqual(MinecraftVersion.V1_13)) {
                    player.sendAsyncPacket(net.minecraft.server.v1_16_R3.PacketPlayOutScoreboardScore(
            ...
            (line until lastLineCount).forEach { i ->
                ...
                if (MinecraftVersion.majorLegacy > 12002) {
                    player.sendAsyncPacket(ClientboundResetScorePacket::class.java.invokeConstructor(uniqueOwner[i], objectiveName))
                    return@forEach
                }
                // 1.13+
                if (MinecraftVersion.isHigherOrEqual(MinecraftVersion.V1_13)) {
